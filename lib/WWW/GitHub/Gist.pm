...
sub show {
	...
	croak 'Missing gist ID' unless $gist;
...
sub list {
	...
	croak 'Missing user ID' unless $user;
...
=for Pod::Coverage get_json_obj post_json_obj patch_json_obj put_json_obj delete_json_obj basic_auth_header 
...
sub basic_auth_header {
	...
	return {} unless ($self -> user and $self -> password);
